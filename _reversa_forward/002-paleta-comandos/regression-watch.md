...
| W006 | `requirements.md` RF-04, RN-05; `roadmap.md` D-06 | A confirmação digita o texto do item e só envia Enter nos itens marcados, após `enterDelayMs`. | presença | `/reversa-requirements ` enviado com Enter, ou comando com barra não executado. |
| W007 | `requirements.md` RF-05, §4 | Lista fixa de 17 itens, de `CONTINUAR` a `/resume`, com Enter ausente só nos três itens terminados em espaço. | redação | Item a mais, a menos, fora de ordem ou com Enter trocado sem nova decisão. |
